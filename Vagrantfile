...
Vagrant.configure("2") do |config|
  ...
  # Set the server hostname to ee-ci-server
  config.vm.hostname = 'ee-ci-server'
  config.vm.network :private_network, ip: '192.168.12.2'
  config.vm.define 'ee-ci-server'
  ...
  config.vm.provision "ansible" do |ansible|
    ...
    # Default galaxy command forces the update of roles, we want to avoid this
